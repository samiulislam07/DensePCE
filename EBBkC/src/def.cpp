...
int HashMap_t::exist(const Edge_t &key) { //Checks if edge exists in hash table, handling both orientations
...
void Heap_t::bubble_up(unsigned int i) { //Standard heap bubble-up operation for maintaining heap property
    unsigned j = (i - 1) >> 1;
    while (i > 0) {
